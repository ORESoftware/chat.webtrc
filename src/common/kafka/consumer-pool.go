// source file path: ./src/common/kafka/consumer-pool.go
package virl_kafka
...
type VirlKafkaConsumer struct {
	conn *kafka.Consumer
	mu   *sync.RWMutex
}

type KafkaConsumerConnPool struct {
	mu          *sync.Mutex
	counter     int
	connections []*VirlKafkaConsumer
	config      *kafka.ConfigMap
	// other fields as necessary, like a channel for signaling, mutex for synchronization, etc.
}

func NewKafkaConsumerConnPool(size int, conf *kafka.ConfigMap) (*KafkaConsumerConnPool, error) {

	pool := &KafkaConsumerConnPool{
		connections: make([]*VirlKafkaConsumer, 0, size),
		config:      conf,
		mu:          &sync.Mutex{},
	}
	...
	for i := 0; i < size; i++ {
		...
		if err := func(i int) error {
			...
			// TODO ping Kafka?
			consumer, err := kafka.NewConsumer(pool.config)
			...
			pool.connections = append(pool.connections, &VirlKafkaConsumer{
				conn: consumer,
				mu:   &sync.RWMutex{},
			})
...
func (p *KafkaConsumerConnPool) GetConnection() (*kafka.Consumer, int) {
	//
	for {
		...
		if true {
			p, err := kafka.NewConsumer(p.config)
			if err != nil {
